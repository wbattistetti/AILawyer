...
export async function extractEvents(text: string, meta?: Record<string, any>, opts?: { timeoutMs?: number }): Promise<{ ok: boolean; events: EventRecord[]; latency_ms?: number }>{
...
export async function extractEventsBatch(items: { text: string; meta?: Record<string,any> }[], opts?: { timeoutMs?: number }): Promise<{ ok: boolean; results: Array<{ ok: boolean; events: EventRecord[]; meta: Record<string,any> }>; latency_ms?: number }>{
	const controller = new AbortController()
	const t = setTimeout(() => controller.abort(), opts?.timeoutMs ?? 800)
	try {
