...
export function AudioViewerAdapter({ file, className = '', onTempFileCreated }: AudioViewerAdapterProps) {
    ...
    return (
      <div className={`h-full flex items-center justify-center ${className}`}>
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">Sto caricando l'audio...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !fileUrl) {
    return (
      <div className={`h-full flex items-center justify-center ${className}`}>
        <div className="text-center">
          <p className="text-sm text-red-600">{error || 'Unable to load audio'}</p>
        </div>
      </div>
    );
  }

  return <MediaViewer file={{ ...file, path: fileUrl, kind: 'audio' }} className={className} />;
}
