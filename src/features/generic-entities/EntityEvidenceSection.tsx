/**
 * Sezione espandibile delle fonti documentali associate a un'entità generica.
 */

import { useMemo, useState } from 'react'
import { ChevronDown, ChevronRight, FileSearch } from 'lucide-react'
import { OccurrenceEvidenceCard } from '../entities/OccurrenceEvidenceCard'
import type { GenericOccurrence } from './types'

type EntityEvidenceSectionProps = {
  occurrences: GenericOccurrence[]
  onOpenOccurrence?: (occurrence: GenericOccurrence) => void
  getPdfUrl?: (occurrence: GenericOccurrence) => string | undefined
  isScanned?: (occurrence: GenericOccurrence) => boolean
...
export function EntityEvidenceSection({
  ...
  const sorted = useMemo(
    () => [...occurrences].sort((left, right) =>
      left.title.localeCompare(right.title) || left.page - right.page
    ),
    [occurrences]
  )
