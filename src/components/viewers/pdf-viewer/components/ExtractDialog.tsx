...
interface ExtractDialogProps {
	...
	onContextMenuChange: (menu: { x: number; y: number; visible: boolean }) => void
...
export const ExtractDialog: React.FC<ExtractDialogProps> = ({
	...
	const handleSave = () => {
		...
		onContextMenuChange({ x: 0, y: 0, visible: false }); // Close context menu
		onOcrInspectOpenChange(false); // Mantieni nascosto OCR inspector
	...
	return (
		<React.Fragment>
			<div className="fixed inset-0 z-[999]" onClick={()=>{ if (!suppressClearRef.current) { onExtractOpenChange(false); } }} />
					...
					<div className="grid grid-cols-1 gap-3">
						...
						<div className="flex items-end gap-2">
							<div className="flex-[2]">
								...
								<select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-[14px] focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors bg-white" value={extractType} onChange={(e)=>onExtractTypeChange(e.target.value)}>
